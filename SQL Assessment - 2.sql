...
select(select count(city) as total_city from station) - (select count(distinct city) as distinct_city from station) as difference;
...
create table delivery(delivery_id int,
					  customer_id int,
                      order_date date,
                      stomer_pref_delivery_date date,
...
/* DATASET FOR QUESTIONS 24 AND 25 */ 
...
create table product1(product_id int,
						description varchar(50),
                        price int,
                        primary key(product_id));

create table orders1(order_id int,
					customer_id int,
                    product_id int,
                    order_date date,
                    quantity int,
...
Return the result table in any order. */
create table tvprogram(program_date datetime,
						content_id int,
                        channel varchar(25),
                        primary key(program_date, content_id));
drop table tvprogram;

create table content(content_id varchar(25),
					title varchar(25),
